...
float gFLDSplit::CrossSections(float nu, int species)
{
  ...
  case 0:  // sigma_HI
    ...
    if (nu < nu0_HI) {
      fprintf(stderr,"gFLDSplit::CrossSections Error: (nu = %g) < %g\n",
	      nu,nu0_HI);
      sigma = -1.0;
      break;
    }
...
//               * POW(1.0+sqrt(y/1.469), -3.188);
  ...
  default:   // illegal input
    fprintf(stderr,"gFLDSplit::CrossSections Error: species %" ISYM" undefined\n",
	    species);
    sigma = -1.0;
  }
