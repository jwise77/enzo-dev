...
int grid::StarParticleCalculateFeedbackVolume(
    Star *cstar, int level, float radius, float DensityUnits, float LengthUnits,
    float VelocityUnits, float TemperatureUnits, float TimeUnits, int &nCells,
    float &depositedMass, float &depositedMetal, float &depositedMetal2,
    FLOAT &depositedVolume) {
  int dim, i, j, k, index;
  int sx, sy, sz;
  FLOAT delx, dely, delz, radius2, Radius, DomainWidth[MAX_DIMENSION];

  if (MyProcessorNumber != ProcessorNumber) return 0.0;

  /* If the radius is less than the cell width, return */

  if (radius < CellWidth[0][0]) return 0.0;
...
************************************************************************/
  ...
  /* Formation section in Grid_AddFeedbackSphere.C uses volume with r=1.0*radius
   */
  if (cstar->ReturnFeedbackFlag() == FORMATION) outerRadius2 = radius * radius;
  /* Feedback uses volume with r=1.2*radius */
  else if (cstar->ReturnFeedbackFlag() == SUPERNOVA)
    outerRadius2 = radius * radius * 1.2 * 1.2;

  // only the volume to actually deposit--counting ghost zones would
  // double-count the volume overlap between grids
  int GZ = NumberOfGhostZones;

  for (k = GZ; k < GridDimension[2] - GZ; k++) {
    delz =
        CellLeftEdge[2][k] + 0.5 * CellWidth[2][k] - cstar->ReturnPosition()[2];
    sz = sign(delz);
    delz = fabs(delz);
    delz = min(delz, DomainWidth[2] - delz);

    for (j = GZ; j < GridDimension[1] - GZ; j++) {
      dely = CellLeftEdge[1][j] + 0.5 * CellWidth[1][j] -
             cstar->ReturnPosition()[1];
      sy = sign(dely);
      dely = fabs(dely);
      ...
      for (i = GZ; i < GridDimension[0] - GZ; i++, index++) {
        delx = CellLeftEdge[0][i] + 0.5 * CellWidth[0][i] -
               cstar->ReturnPosition()[0];
        sx = sign(delx);
        delx = fabs(delx);
        ...
        if (radius2 <= outerRadius2) {
          ...
          if (cstar->ReturnFeedbackFlag() == FORMATION) {
            depositedMass += BaryonField[DensNum][index] * CellWidth[0][i] *
                             CellWidth[1][j] * CellWidth[2][k];
            if (SNColourNum > 0)
              depositedMetal += BaryonField[SNColourNum][index] *
                                CellWidth[0][i] * CellWidth[1][j] *
                                CellWidth[2][k];
            if (cstar->ReturnType() != PopIII && MetalNum > 0)
              depositedMetal2 += BaryonField[MetalNum][index] *
                                 CellWidth[0][i] * CellWidth[1][j] *
                                 CellWidth[2][k];
          }
