...
int grid::CopyMonteCarloTracerParticlesFromGrid(grid *OtherGrid, FLOAT EdgeOffset[MAX_DIMENSION])
{
  ...
  this->DebugCheck("CopyMonteCarloTracerParticlesFromGrid (before)");
  ...
  // *** TODO: Implement CommunicationMonteCarloTracerParticles
  ...
  // *** MOVE MC TRACER HERE ***
  if (!isShearing)
    printf("\nMove MC Tracers HERE\n");

   //Update the energys due to sheared boundaries
  if (isShearing){
    fprintf(stderr, "%s\n", "Shearing Boundary Conditions not implemented for MC tracers!");  
  }
  ...
  this->DebugCheck("CopyMonteCarloTracerParticlesFromGrid (after)");
