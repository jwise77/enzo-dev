...
#define NFW_POINTS 500
 ...
 /**** ^^^ PROBABLY DONT NEED TO KEEP THIS ^^^ ******/
  ...
  } /* End SecondPass to initialize star particle */
  ...
  //    float vel = sqrt(PointSourceGravityConstant/drad)/drad;
    ...
    if (r < outer_radius) {
      ...
      double cs = sqrt(Gamma * kboltz * temperature / (mu_data * mh)) / (LengthUnits/TimeUnits);
      //printf("mu    = %f\n", mu_data);
      //printf("gamma = %f\n", Gamma);
      //printf("T     = %f\n", temperature);      
      //printf("cs    = %f\n", cs * (LengthUnits/TimeUnits));

      double vx, vy, vz, vmag;
      vx = SphereTurbulence * tsf_Maxwellian(temperature, LengthUnits/TimeUnits, mu_data);
      vy = SphereTurbulence * tsf_Maxwellian(temperature, LengthUnits/TimeUnits, mu_data);
      vz = SphereTurbulence * tsf_Maxwellian(temperature, LengthUnits/TimeUnits, mu_data);
      vmag = sqrt(pow(vx,2) + pow(vy,2) + pow(vz,2));
      //printf("vmag  = %f\n", vmag * (LengthUnits/TimeUnits));
      //// limit to sound speed
      //if (vmag > cs){
      //  vx *= cs/vmag;
      //  vy *= cs/vmag;
      //  vz *= cs/vmag;
...
double tsf_Maxwellian(double T, double vel_unit, double mu)
{
   // Compute random Maxwellian velocity (1/3 sound speed)
   double mean = 0;
   double stdev = sqrt((kboltz*T)/(mu*mh))/3;
   double u1 = rand();
   u1 = u1/RAND_MAX;
   double u2 = rand();
   u2 = u2/RAND_MAX;
